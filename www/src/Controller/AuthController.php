...
class AuthController extends Controller
{
    ...
    public function sign_in(Request $request) {
        try{
            ...
            //Validation
            $is_valid = true;
            $is_valid = $this->validator->validateEmail($email) && $is_valid;
            $is_valid = $this->validator->validatePassword($password)  && $is_valid;
            $is_valid = $this->validator->validateName($firstname, 'firstname, true, 100') && $is_valid;
            $is_valid = $this->validator->validateName($lastname, 'lastname, true, 100') && $is_valid;

            if(!$is_valid){
                $error = $this->validator-> getFirstError() ?? "Les données fournies ne sont pas valides";
